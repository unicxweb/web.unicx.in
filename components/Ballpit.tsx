...
function onTouchStart(e: TouchEvent) {
  if (e.touches.length > 0) {
    ...
    const isInteractive = target && (target.closest('button') || target.closest('a') || (target.closest('.pointer-events-auto') && !target.closest('canvas')));
    if (!isInteractive) {
      e.preventDefault();
    }
...
function createBallpit(canvas: HTMLCanvasElement, config: any = {}): CreateBallpitReturn {
  console.log('createBallpit called with config', config);
  ...
  console.log('Spheres initialized');
  ...
  function initialize(cfg: any) {
    console.log('initialize called with cfg', cfg);
    if (spheres) {
      threeInstance.clear();
...
const Ballpit: React.FC<BallpitProps> = ({ className = '', followCursor = true, count = 100, gravity = 0.01, friction = 0.9975, wallBounce = 0.95, colors = [0x000000], ...props }) => {
  ...
  useEffect(() => {
    ...
    console.log('Ballpit: Initializing with canvas', canvas);
    ...
    try {
      ...
      console.error('Ballpit: Initialization error', error);
    }
  ...
  return <canvas className={className} ref={canvasRef} style={{ width: '100%', height: '100%', display: 'block', background: 'transparent' }} />;
};
