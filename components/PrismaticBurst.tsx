...
void main(){
    ...
    // Construct 3D rotation matrix outside the loop to avoid redundant branching and computations
    mat3 R = mat3(1.0);
    if(uAnimType == 0){
        R = mat3(
            c.x, 0.0, c.z,
        ...
        R = rotZ(ang.z) * rotY(ang.y) * rotX(ang.x);
    } else if(uAnimType == 2){
        vec2 m = uMouse * 2.0 - 1.0;
        vec3 ang = vec3(m.y * 0.6, m.x * 0.6, 0.0);
        R = rotY(ang.y) * rotX(ang.x);
    }

    vec3 rotDir = R * dir;
    vec3 rotOffset = R * vec3(0.0, 0.0, 2.0);

    // Optimized 44 steps with rotation hoisting and distort branch elimination
    for (int i = 0; i < 44; ++i) {
        vec3 P = marchT * rotDir - rotOffset;
        float rad = length(P);
        ...
        // Skip heavy bend angle calculations when uDistort is 0
        if (amp > 0.0) {
...
const PrismaticBurst = ({
  ...
  const startLoop = () => {
    ...
    const update = (now: number) => {
      const dt = Math.max(0, now - lastTimeRef.current) * 0.001;
      ...
      if (currentProgram && currentRenderer && currentMesh) {
        currentProgram.uniforms.uMouse.value = sm as any;
        currentProgram.uniforms.uTime.value = accumTimeRef.current;
  ...
  useEffect(() => {
    ...
    if (color0 || color1 || color2 || color3 || color4 || color5) {
      if (color0) activeColors.push(color0);
      if (color1) activeColors.push(color1);
      if (color2) activeColors.push(color2);
      if (color3) activeColors.push(color3);
      if (color4) activeColors.push(color4);
      if (color5) activeColors.push(color5);
    } else if (Array.isArray(colors) && colors.length > 0) {
      activeColors.push(...colors);
    }
    ...
    // Single frame render if currently paused to reflect parameter/color changes instantly
    if (pausedRef.current && meshRef.current) {
      renderer.render({ scene: meshRef.current });
    }
  }, [intensity, speed, animationType, colors, color0, color1, color2, color3, color4, color5, distort, offset, rayCount]);
