/**
 * Careers Image Compression Pipeline
 * 
 * Generates optimized WebP versions of all images in /public/careers/
 * Run: node compress-careers.mjs
 * 
 * Output structure:
 *   /public/careers/optimized/
 *     ├── image-name.webp          (max 1200px wide, quality 78)
 *     ├── image-name-mobile.webp   (max 640px wide, quality 72)
 *     └── ...
 */
...
const INPUT_DIR = "raw-images/careers";
const OUTPUT_DIR = "public/careers/optimized";
...
async function main() {
  console.log("🔧 Careers Image Compression Pipeline\n");

  // Ensure output directory exists
  await mkdir(OUTPUT_DIR, { recursive: true });

  const files = await getImageFiles(INPUT_DIR);
  console.log(`Found ${files.length} images in ${INPUT_DIR} to process.\n`);
  ...
  results
    ...
    .forEach((r) => {
      console.log(`  ${r.name.padEnd(45)} ${r.inputSize.padStart(8)} KB → ${r.outputSize.padStart(8)} KB  (${r.savings})`);
    });
}
